...
export function CalendarFooter({ items, isLoading, isError, onItemClick, onEmptyActionClick }: CalendarFooterProps) {
  if (isLoading) {
    return (
      <div className="rounded-[10px] bg-gray-100 px-5 py-4">
        <p className="body-m-medium text-gray-500">면접 일정을 불러오는 중이에요.</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="rounded-[10px] bg-gray-100 px-5 py-4">
        <p className="body-m-medium text-gray-500">면접 일정을 불러오지 못했어요.</p>
      </div>
    )
  }
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center gap-1.5 rounded-[10px] bg-gray-100 px-5 py-6">
        ...
        <p className="body-s-medium text-gray-500">해당 일자의 면접 정보가 없어요</p>
        <Button
          type="button"
