...
export const getGetPdfHighlightingsUrl = (qnaSetId: number) => {
  return `/qna-set/${qnaSetId}/pdf-highlighting`
}
...
export const getGetPdfHighlightingsQueryKey = (qnaSetId: number) => {
  return [`/qna-set/${qnaSetId}/pdf-highlighting`] as const
}
...
/**
 * @summary 지정한 질문 답변 세트의 PDF 하이라이팅을 모두 삭제합니다.
 */
export const getDeletePdfHighlightingUrl = (qnaSetId: number) => {
  return `/qna-set/${qnaSetId}/pdf-highlighting`
}

export const deletePdfHighlighting = async (qnaSetId: number, options?: RequestInit): Promise<ApiResponseVoid> => {
  return customFetch<ApiResponseVoid>(getDeletePdfHighlightingUrl(qnaSetId), {
    ...options,
    method: 'DELETE',
  })
}

export const getDeletePdfHighlightingMutationOptions = <TError = unknown, TContext = unknown>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deletePdfHighlighting>>,
    TError,
    { qnaSetId: number },
    TContext
  >
  request?: SecondParameter<typeof customFetch>
}): UseMutationOptions<Awaited<ReturnType<typeof deletePdfHighlighting>>, TError, { qnaSetId: number }, TContext> => {
  const mutationKey = ['deletePdfHighlighting']
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation && 'mutationKey' in options.mutation && options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
  ...
  const mutationFn: MutationFunction<Awaited<ReturnType<typeof deletePdfHighlighting>>, { qnaSetId: number }> = (
    props,
  ) => {
    const { qnaSetId } = props ?? {}

    return deletePdfHighlighting(qnaSetId, requestOptions)
  }

  return { mutationFn, ...mutationOptions }
}

export type DeletePdfHighlightingMutationResult = NonNullable<Awaited<ReturnType<typeof deletePdfHighlighting>>>

export type DeletePdfHighlightingMutationError = unknown

/**
 * @summary 지정한 질문 답변 세트의 PDF 하이라이팅을 모두 삭제합니다.
 */
export const useDeletePdfHighlighting = <TError = unknown, TContext = unknown>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof deletePdfHighlighting>>,
      TError,
  ...
  queryClient?: QueryClient,
): UseMutationResult<Awaited<ReturnType<typeof deletePdfHighlighting>>, TError, { qnaSetId: number }, TContext> => {
  return useMutation(getDeletePdfHighlightingMutationOptions(options), queryClient)
}
/**
