...
public class QnaSetCustomRepositoryImpl implements QnaSetCustomRepository {
    ...
    @Override
    public Page<QnaSet> searchByIndustriesAndJobCategories(List<Long> industryIds, List<Long> jobCategoryIds, Pageable pageable) {
        List<QnaSet> qnaSets = queryFactory
                .selectFrom(qnaSet)
                .where(Expressions.asBoolean(true).isTrue(),
                        industryIds == null || industryIds.isEmpty() ? null : qnaSet.interview.industry.id.in(industryIds),
                       jobCategoryIds == null || jobCategoryIds.isEmpty() ? null : qnaSet.interview.jobCategory.id.in(jobCategoryIds))
